...
export const CryptoWallets: React.FC<CryptoWalletsProps> = ({
  ...
  if (!wallets || Object.keys(wallets).length === 0 || !defaultNetwork) {
    return null; // Ничего не рендерим, если кошельков нет
  }

  const primaryAddress = wallets[defaultNetwork]?.address || "";
  const currentAddress = (activeNetwork && wallets[activeNetwork]?.address) || primaryAddress;
  ...
  return (
    <>
      ...
      <Button
        ...
        variant="flat"
        size="sm"
        className={`px-3 flex shrink-0 gap-1 font-mono tracking-wider items-center font-medium h-7 min-h-7 relative z-10 ${className}`}
        onPress={onOpen}
      >
