...
export function MobileDocsNav({ tree }: { tree: Record<string, any> }) {
  ...
  return (
    <div 
      ref={menuRef} 
      className="w-full sticky top-[64px] sm:top-[72px] z-[40] bg-background lg:hidden mb-6 border-b border-primary rounded-none"
    >
      <button 
      ...
      >
        ...
        <svg className={`shrink-0 ml-3 transition-transform duration-300 text-primary ${isOpen ? 'rotate-180' : ''}`} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
